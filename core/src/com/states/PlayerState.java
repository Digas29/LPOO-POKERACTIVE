...
public class PlayerState implements GameState {
		...
		showCards.addListener(new ClickListener(){
			...
			public boolean touchDown(InputEvent event, float x, float y,
				...
				if(!showCards.isDisabled()){
					tableL.clear();
					tableR.clear();
					Texture t1 = new Texture(Gdx.files.internal(cardFileName(player.getCards().get(0))));
					Texture t2 = new Texture(Gdx.files.internal(cardFileName(player.getCards().get(1))));
	...
	public static String cardFileName(Card card){
		return "img/cards/" + Card.rankAsString(card.getRank()) + "_of_" + Card.suitAsString(card.getSuit()) + ".png";
	}
