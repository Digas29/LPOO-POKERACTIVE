...
public class BoardState implements GameState{
	...
	public BoardState(){
		temp = new NinePatch(new Texture(Gdx.files.internal("img/black.png")), 10, 10, 10, 10);
	...
	private void showBoard() {
		final Card[] cards= game.getActualBoard();
		if(cards[3] == null){
			new Thread(new Runnable() {
				   ...
				   public void run() {
				      Gdx.app.postRunnable(new Runnable() {
				         @Override
				         public void run() {
				        	Texture t1 = new Texture(Gdx.files.internal(PlayerState.cardFileName(cards[0])));
				 			Texture t2 = new Texture(Gdx.files.internal(PlayerState.cardFileName(cards[1])));
				 			Texture t3 = new Texture(Gdx.files.internal(PlayerState.cardFileName(cards[2])));
				 			Image img1 = new Image(t1);
				 			Image img2 = new Image(t2);
				 			Image img3 = new Image(t3);
			...
			new Thread(new Runnable() {
				   ...
				   public void run() {
				      Gdx.app.postRunnable(new Runnable() {
				         @Override
				         public void run() {
				        	Texture t1 = new Texture(Gdx.files.internal(PlayerState.cardFileName(cards[3])));
				 			Image img4 = new Image(t1);
			...
			new Thread(new Runnable() {
				   ...
				   public void run() {
				      Gdx.app.postRunnable(new Runnable() {
				         @Override
				         public void run() {
				        	Texture t1 = new Texture(Gdx.files.internal(PlayerState.cardFileName(cards[4])));
				 			Image img5 = new Image(t1);
	...
	public void showFrontCards(){
		...
		for(int i = 0; i < game.getPlayers().size(); i++){
			final int pos = i;
			final Player p= game.getPlayers().get(i);
			if(p.inGame()){
				new Thread(new Runnable() {
					   @Override
					   public void run() {
					      Gdx.app.postRunnable(new Runnable() {
					         @Override
					         public void run() {
					        	 Texture t1 = new Texture(Gdx.files.internal(PlayerState.cardFileName(p.getCards().get(0))));
									Texture t2 = new Texture(Gdx.files.internal(PlayerState.cardFileName(p.getCards().get(1))));
									Image img1 = new Image(t1);
									Image img2 = new Image(t2);
									img1.setSize(99.0f, 165.0f);
									img2.setSize(99.0f, 165.0f);
